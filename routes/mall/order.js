                  ...
                  connection.query(
                    ...
                    function (error, results, fields) {
                      if (error || results.affectedRows <= 0) {
                        return connection.rollback(function () {
                          throw (
                            error || `${results.affectedRows} rows affected!`
                          );
                        });
                      }
...
router.get("/list", function (req, res) {
  ...
  // 查询所有订单
  let sql = `SELECT o.id, o.create_time, o.payment, os.text AS status
		 FROM orders o JOIN order_status os ON o.order_state = os.CODE
		 WHERE o.uid = ? ORDER BY o.create_time DESC LIMIT ? OFFSET ? `;
  // 根据订单状态查询
  if (status != "all") {
    sql = `SELECT o.id, o.create_time, o.payment, os.text AS status
			 FROM orders o JOIN order_status os ON o.order_state = os.CODE
